    ...
    , ui(new Ui::RegisterDialog),_countdown(5)
{
    ...
    ui->user_edit->setEchoMode(QLineEdit::Normal);
...
void RegisterDialog::on_getbtn_clicked()
{
    ...
    bool match=regex.match(email).hasMatch();//匹配邮箱
    if(match)
    {
        //发送http验证码
...
void RegisterDialog::slot_reg_mod_finish(ReqId id, QString res, ErrorCode err)
{
    ...
    if(err!=ErrorCode::SUCCESS)
    {
        if(res.isEmpty())
        {
            showTip(tr("服务器返回空响应，请检查服务器状态"), false);
        }
...
void RegisterDialog::ChangeTipPage()
{
    ...
    if (!ui->stackedWidget || !ui->page_2) {
        qDebug() << "stackedWidget or page_2 is null!";
        return;
    }
    //启动定时器
...
void RegisterDialog::on_sure_btn_clicked()
{
    ...
    QString registerUrl = gate_url_prefix + "/user_register";
    qDebug() << "注册请求 URL: " << registerUrl;
    HttpMgr::GetInstance()->PostHttpReq(QUrl(gate_url_prefix+"/user_register"),
                                        json_obj, ReqId::ID_REG_USER,Modules::REGISTERMOD);
    qDebug() << "注册请求参数: " << QJsonDocument(json_obj).toJson(QJsonDocument::Compact);
}
